...
export function authReducer(state: AuthState = InitialAuthState, action: authActions.Actions): AuthState {
    switch (action.type) {
        case authActions.AuthActions.REGISTER_SUCCESS:
            return { ...state, firstName:action.payload.firstname, lastName: action.payload.lastname, email: action.payload.email, refreshToken: action.payload.refreshToken, uid: action.payload.uid, image: action.payload.image, errors: [] }
